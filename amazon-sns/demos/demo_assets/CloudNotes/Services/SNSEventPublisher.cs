...
using CloudNotes.Models.MobilePush;
    ...
    public class SNSEventPublisher : IEventPublisher
    {
        ...
        public async Task PublishEvent(Event eventData)
        {
            try
            {
                ...
                message.SMSContent = 
                    $"{eventData.EventType.ToString()} - Note {eventData.NoteId}";
                message.FCMContent = new FCMNotification()
                {
                    Notification = new MobilePushNotification(
                        eventData.EventType.ToString(),
                        $"New event for note {eventData.NoteId}"
                        ),
